#!/usr/bin/perl -w
=head1 #===============================================================================
#        USAGE: perl GSEA_prepare.pl <Exp.mat> <Sample_info.txt> <Gene_set.gmt> <outdir> <database> [Design.txt]
#
...
#===============================================================================
=cut
die `pod2text $0` unless @ARGV == 5 or @ARGV == 6;
use strict;
...
sub exp_extact{
	...
	push @sample_list, @{$hash2{$tt[1]}};
        ...
        print OUT2 "\n";
        close OUT2;
	print "gseapy gsea -p 20 -d $outdir/$tt[0]_vs_$tt[1]/exp.mat -c $outdir/$tt[0]_vs_$tt[1]/anno.cls -g $gmt -o $outdir/$tt[0]_vs_$tt[1]/$database\n";
#	print "sed \'s/,/\\t/g\' $outdir/$tt[0]_vs_$tt[1]/gseapy.gsea.gene_set.report.csv > $outdir/$tt[0]_vs_$tt[1]/GSEA.gene_set.report.xls\n";
	print "less $outdir/$tt[0]_vs_$tt[1]/$database/gseapy.gsea.gene_set.report.csv | perl -wanle\'\@F=split /,/;my\$a=join \",\", \@F[7..\$#F];print join \"\\t\",\@F[0..6], \$a\' > $outdir/$tt[0]_vs_$tt[1]/$database/GSEA.gene_set.report.xls\n";
}
